...
const CACHE_NAME = 'multiradios-v1.1.0';
const ASSETS = [
  './',
  './index.html',
  '',
...
self.addEventListener('fetch', (event) => {
  ...
  // Never cache audio streams (live radio) ni las llamadas a la API (config global)
  if (req.destination === 'audio' || url.pathname.includes('/stream') || url.pathname.includes('/live') || req.url.includes('/api/')) {
    return;
  }
